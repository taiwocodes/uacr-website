...
export const Testimonials = () =>{
    return(
        <Box bg="blue.100" h="34rem" className="relative">
                <Image src={bgStar4} className="absolute top-14 left-4" />
                <Image src={bgLine3} className="absolute right-0 bottom-0" />
                <Box className="absolute w-screen top-0 py-[70px] z-10 overflow-x-hidden">
                  <Text color="gray.90" className="pl-16">What do people think?</Text>
                  <Text fontSize="2rem" fontWeight="bold" className="pl-16">Testimonials</Text>
                  <HStack w="100%" gap={8} wrap="nowrap" className="pl-16 pr-4 py-4 overflow-x-auto">
                    {Array.from({length:5}).map((_) => (
                      testimonials.map((testiment) => (
                      <Flex direction="column" justifyContent="space-between" bg="white" borderWidth="thin" borderColor="gray.50" minW="24rem" h="17rem"  className=" rounded-lg p-4 ">
